...
VBulletPlayer = 20  # Tốc độ Bullet
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    ...
                    if button_2.collidepoint((mx, my)):
                        hs_run = True
                        show_texts_middle(50, "Leader Board", 200)
                        highScores()
                        continue
        pygame.display.update()  # Update
    pygame.display.update()  # Update
...
def highScores():
    ...
    while hs_run:
        ...
        for index, sc in enumerate(hs):
            show_texts(250, 200 + index * 50, f"{index + 1}.     {sc[0]}    {sc[1]}", 25)
...
def run():
    ...
    while gameRunning:
        ...
        if len(listEnemy) == 0:
            ...
            for i in range(wave_length):
                enemy = Enemy(random.randrange(50, xScreen - 100), random.randrange(-1500, -100),
                              random.choice(["default"]), 80, 80, VBulletEnemy)
        ...
        player.move_lasers(-VBulletPlayer, listEnemy)
        ...
        if lost:  # Nếu thua
            ...
            while True:
                ...
                global yes_box
                global no_box
                input_box = InputBox(200, yScreen / 2 + 40, 140, 32)
                if len(hs) == 0 or hs[len(hs) - 1][1] < scores and not yes_box and not no_box:
                    screen.blit(background, (0, 0))  # vẽ lại background
                    show_texts_middle(100, f"Scores:{scores}", 60)  # In điểm
                    show_texts_middle(yScreen / 2 - 100, "GAME OVER", 70)  # In Thông báo thua
                    show_texts_middle(yScreen / 2, "Do you want to save your scores?", 40)
                    button_yes = pygame.Rect((xScreen / 2 - 150, yScreen / 2 + 50, 120, 50))
                    button_no = pygame.Rect((xScreen / 2 + 30, yScreen / 2 + 50, 120, 50))
                    pygame.draw.rect(screen, (51, 153, 255), button_yes)
                    pygame.draw.rect(screen, (255, 77, 77), button_no)
                    show_text_menu(xScreen / 2 - 90, yScreen / 2 + 50 + 25, "Yes", 25)
                    show_text_menu(xScreen / 2 + 90, yScreen / 2 + 50 + 25, "No", 25)
                    for event in pygame.event.get():  # Nếu nhấn
                        if event.type == pygame.QUIT:  # Thoát
                            pygame.quit()
                            quit()
                    ...
                    screen.blit(background, (0, 0))  # vẽ lại background
                    show_texts_middle(100, f"Scores:{scores}", 60)
                    show_texts_middle(yScreen / 2 - 100, "GAME OVER", 70)
